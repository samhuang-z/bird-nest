...
def pod = libraryResource 'io/milvus/pod/rte.yaml'
            ...
            steps {
                container('jnlp') {
                    script {
                            ...
                            def childJob =  build job: "Milvus Build Images/x86/cpu/${env.BRANCH_NAME}", wait: true
                            copyArtifacts(projectName: "Milvus Build Images/x86/cpu/${env.BRANCH_NAME}", selector: specific("${childJob.number}"))

                            output = readJSON file: 'output.json'
                            tag = output.image.tag
                            
                            
              
                            sh "echo ${tag} > imageTag.txt"
